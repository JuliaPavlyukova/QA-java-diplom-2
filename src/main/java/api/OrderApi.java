...
public class OrderApi {
    ...
    public ValidatableResponse createOrderWithAuth(Order order, String token) {
        return requestSpecification().header("Authorization", token).body(order).when().post(Constants.ORDERS_REQUEST).then();
    }
    ...
    public ValidatableResponse createOrderWithoutAuth(Order order) {
        return requestSpecification().body(order).when().post(Constants.ORDERS_REQUEST).then().log().all();
    }
    ...
    public ValidatableResponse getAvailableIngredients() {
        return requestSpecification().when().get("/api/ingredients").then();
    }
    ...
    public ValidatableResponse getOrders(String token) {
        if (token != null) {
            return requestSpecification().header("Authorization", token).when().get(Constants.ORDERS_REQUEST).then();
        } else {
            return requestSpecification().when().get(Constants.ORDERS_REQUEST).then();
        }
    }
}
